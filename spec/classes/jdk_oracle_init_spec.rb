...
describe 'jdk_oracle', :type => 'class' do
    ...
    context 'When deploying on CentOS' do
        ...
        context 'with default parameters' do
            it {
                should contain_exec( 'get_jdk_installer').with_creates('/tmp/jdk-8u131-linux-x64.rpm')
                should contain_file('/tmp/jdk-8u131-linux-x64.rpm')
                should contain_exec('install_rpm').with_creates('/usr/java/jdk1.8.0_131')
